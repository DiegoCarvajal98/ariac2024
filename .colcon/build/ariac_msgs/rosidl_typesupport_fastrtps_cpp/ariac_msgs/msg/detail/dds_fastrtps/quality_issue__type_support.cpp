  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = ariac_msgs::msg::QualityIssue;
    is_plain =
      (
      offsetof(DataType, incorrect_part_color) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _QualityIssue__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_QualityIssue(full_bounded, is_plain, 0);
