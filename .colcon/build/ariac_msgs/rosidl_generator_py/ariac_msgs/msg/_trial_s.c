...
bool ariac_msgs__msg__trial__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // log_folder_path
    PyObject * field = PyObject_GetAttrString(_pymsg, "log_folder_path");
    if (!field) {
      return false;
    }
    assert(PyUnicode_Check(field));
    ...
    rosidl_runtime_c__String__assign(&ros_message->log_folder_path, PyBytes_AS_STRING(encoded_field));
    Py_DECREF(encoded_field);
    Py_DECREF(field);
  }
  {  // orders
...
PyObject * ariac_msgs__msg__trial__convert_to_py(void * raw_ros_message)
{
  ...
  {  // log_folder_path
    PyObject * field = NULL;
    field = PyUnicode_DecodeUTF8(
      ros_message->log_folder_path.data,
      strlen(ros_message->log_folder_path.data),
      "replace");
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "log_folder_path", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
